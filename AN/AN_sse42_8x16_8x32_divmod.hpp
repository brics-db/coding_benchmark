...
struct AN_sse42_8x16_8x32_divmod : public ANTest<uint16_t, uint32_t, UNROLL>, public SSE42Test
{

	AN_sse42_8x16_8x32_divmod(const char* const name, AlignedBlock & in, AlignedBlock & out, uint32_t A = 63'877ul, uint32_t Ainv = 3'510'769'485ul) :
			ANTest<uint16_t, uint32_t, UNROLL>(name, in, out, A, Ainv) { }

	virtual
	~AN_sse42_8x16_8x32_divmod() { }
		...
		for (size_t iteration = 0; iteration < numIterations; ++iteration)
		{
			...
			while (data <= (dataEnd - UNROLL))
			{
				...
				for (size_t k = 0; k < UNROLL; ++k)
				{
					...
					// auto mm_unenc = 
					if ((_mm_extract_epi32(mmIn, 0) % this->A != 0) || (_mm_extract_epi32(mmIn, 1) % this->A != 0) || (_mm_extract_epi32(mmIn, 2) % this->A != 0) || (_mm_extract_epi32(mmIn, 3) % this->A != 0))
