...
namespace coding_benchmark {
    ...
    struct Hamming_avx512 :
public Test<DATAIN, hamming_t<DATAIN, __m512i >>,
public AVX2Test {

        typedef hamming_t<DATAIN, __m512i > hamming_avx512_t;
        typedef hamming_t<DATAIN, DATAIN> hamming_scalar_t;

        using Test<DATAIN, hamming_avx512_t>::Test;

        virtual ~Hamming_avx512() {
        }
        ...
        void RunEncode(
            ...
            for (size_t iteration = 0; iteration < config.numIterations; ++iteration) {
                _ReadWriteBarrier();
                auto data = this->bufRaw.template begin<__m512i >();
                auto dataEnd = this->bufRaw.template end<__m512i >();
        ...
        void RunCheck(
            ...
            for (size_t iteration = 0; iteration < config.numIterations; ++iteration) {
                _ReadWriteBarrier();
                const constexpr size_t VALUES_PER_VECTOR = sizeof(__m512i) / sizeof (DATAIN);
        ...
        struct Arithmetor {
            typedef hamming_t<DATAIN, __m512i> hamming_avx2_t;
            typedef hamming_t<DATAIN, DATAIN> hamming_scalar_t;
            Hamming_avx2 & test;
            ...
            void impl() {
                ...
                auto mmOperand = SIMD<__m512i, DATAIN>::set1(config.operand);
                size_t i = 0;
        ...
        void RunDecode(
            ...
            for (size_t iteration = 0; iteration < config.numIterations; ++iteration) {
                ...
                size_t i = 0;
                auto data = this->bufEncoded.template begin<hamming_avx2_t>();
                auto dataOut = this->bufResult.template begin<__m512i >();
                while (i <= (numValues - VALUES_PER_UNROLL)) {
